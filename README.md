...
- 阅读导航样式
  ...
  - `Barcode`：适合沉浸式长文阅读，用透明 rail 与短横线表达章节位置，停留 180ms 后预览标题，滚轮保留局部 wave 反馈
    - `滚轮`：标题在窄幅固定观察窗内滚动，只为当前项提供低成本焦点带；桌面宽 242px，移动端收敛到 210px
    - `聚光灯`：hover 时显示当前项上下各 2 项；当前标题可显示两行，邻项逐级淡出，与滚轮形成清晰的静态上下文差异
    - `GPT`：idle 保留纯条形码；hover 时展开带 OUTLINE 标题、条目数量和高对比当前位置标记的完整可滚动面板
    - `少数派`：idle 保留固定在正文外侧或屏幕边缘的 2px 竖刻度；hover 后展开完整标题且刻度不横移，当前章节与键盘焦点统一使用少数派红，并可固定或释放大纲

- 低干扰与性能约束
  - 默认状态只保留低透明度入口或 rail，预览仅在主动 hover、聚焦、点击或固定后增强
  - 动态标题路径只更新必要条目，并优先使用 `transform` / `opacity`；所有主题均不叠加 `backdrop-filter`，正文指针移动不触发回顶动画帧
  - 每种主题只使用中性色和一种功能强调色；少数派的当前章节、焦点与固定状态统一使用明暗自适应红色
