...
- `阅读进度目录`
  - 适合内容型长文
  - 强调低侵扰位置感知、hover wave、标题预览和轻量回顶

## 2. 推荐短描述

用于 Chrome 商店短描述，尽量控制在 132 个字符以内：

> Adaptive reading navigation for long articles and docs: TOC, progress outline, hover preview, smart skip, and smooth scroll to top.
...
- Reading Progress Outline
  - Great for immersive long-form reading
  - Uses subtle rail markers to show where you are in the article
  - Shows a lightweight hover preview outside the rail
  - Adapts gently to nearby page surfaces while keeping the rail transparent
...
- Subtle local color adaptation for the progress outline
...
- 阅读进度目录
  - 适合沉浸式长文阅读
  - 默认以低侵扰短横线表达章节位置
  - 悬停时短横线向外延展，并在外侧显示标题预览
  - 基于附近页面背景做轻量自适应，同时保持 rail 本体透明
...
2. `阅读进度目录` 在长文页
   - 展示透明短横线默认态、hover wave、标题预览和当前章节高亮
3. 选项页
...
> Improved the progress-outline reading mode with smoother hover waves, fixed-position title previews, subtle local color adaptation, and better left/right placement.
