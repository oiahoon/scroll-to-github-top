...
默认不填写 `release_tag` 时，手动触发只生成 Artifact，不会创建 GitHub Release。如果 Tag 自动发布曾失败，可以填写现有 Tag（例如 `v2.14`）安全重试；工作流会 checkout 该 Tag，确保附件内容与 Tag 对应源码一致。

## 创建 GitHub Release

Tag 必须严格等于 `v` 加上 `manifest.json` 的版本号。例如 Manifest 版本为 `2.14` 时：
...
工作流会拒绝 `v2.15` 与 Manifest `2.14` 这类版本不一致的发布。校验通过后，它会：
...
`2.14` 建议粘贴 `CHROME_STORE_LISTING.md` 中的 `Release Note — v2.14`；上传前确认 ZIP 文件名为 `smart-toc-scroll-2.14.zip`，且商店后台解析出的版本号为 `2.14`。
