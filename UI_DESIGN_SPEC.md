...
> **版本**：v2.16 当前落地参考版
> **更新日期**：2026-08-28
> **说明**：本文保留 v2.2 基线问题分析，并汇总 v2.5–v2.16 已落地的 rail、主题和 Options 视觉决策；标注“需补充”的条目仍属于后续 backlog。
> **适用文件**：toc.css、themes.css、options.html、options.css

### v2.16 功能性设计守则

1. 阅读优先：默认态只保留必要入口或进度刻度，增强信息仅在 hover、focus、click 或 pinned 状态出现。
2. 正文优先：预览优先占用正文外侧留白；桌面 Wheel 上限 242px、移动端 210px，SSPAI 宽度服从实际可用边距。
3. 性能优先：Wheel 与 Spotlight 高频标题层禁止使用 `backdrop-filter`；连续反馈优先使用 transform / opacity。
4. 颜色克制：仅使用页面自适应中性色、功能蓝与 SSPAI 红；颜色必须表达当前、焦点、固定或可操作状态。
5. 差异来自任务：Wheel 表达连续滚动定位，Spotlight 表达稳定的五项上下文，GPT 提供完整目录，SSPAI 提供可固定的边缘大纲。
