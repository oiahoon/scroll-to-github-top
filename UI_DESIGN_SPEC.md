...
> **版本**：v2.18 当前设计契约
> **更新日期**：2026-09-05
> **说明**：以下 v2.18 契约优先于后文历史规范；旧 Wheel / Spotlight / GPT / SSPAI 描述保留用于追溯，不作为当前实现要求。
> **适用文件**：catalog.js、theme.js、toc.css、themes.css、options.html、options.js、options.css

### v2.18 按任务区分模式

| 模式 | 解决的任务 | 信息量与退出方式 | 兼容键 |
| --- | --- | --- | --- |
| 标准面板 | 偶尔跳转到另一节 | 按需完整目录，Close / Esc | default |
| 邻节预览 | 确认附近的跳转目标 | 目标与前后各一项，移开 / Esc | spotlight，wheel 自动迁移 |
| 检索目录 | 已知关键词，查找长文内容 | 本地筛选完整标题，方向键 / Enter / Esc | gpt |
| 页边大纲 | 阅读时反复对照结构 | 可固定的页边目录，图钉 / Esc | sspai |

合并依据：原 Wheel 与 Spotlight 都以五项上下文解决附近定位，差异主要来自滚动与波形；保留两套增加选择与维护成本。Standard 与 GPT 原本都为完整目录，因此为后者补上本地检索任务。当前四种模式已覆盖实际需求，本轮不新增仅靠颜色或动效区分的主题。

1. 默认阅读态低存在感，180ms 悬停意图判断；键盘立即响应，不自动抢走文章焦点。
2. 每种模式只用中性色与一种功能强调色；不使用循环动效和背景模糊。
3. 邻节预览最多三个复用节点，不按文章标题数创建第二份完整预览；移除旧滚轮轨道和逐项波动热路径。
4. 检索只扫描已提取的标题文本，不重新扫描正文或联网；支持 NFKC、大小写和多词匹配，零结果可恢复。
5. 标准、检索和页边目录长标题换行；邻节预览两行限制，完整标题仍由对应链接的无障碍名称提供。
6. 保留左右布局、窄屏限制、减少动态效果、Esc 清理与旧偏好迁移。真实性能和站点兼容性需要在相应宿主验证，不能由本地样本推定。
...
*本规范基于 Smart TOC & Scroll v2.2 基线问题分析形成，并已按 v2.16 当前落地状态整理，日期：2026-08-28。*
